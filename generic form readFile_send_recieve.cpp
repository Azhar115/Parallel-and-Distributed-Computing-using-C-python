...
template<typename T>
int search_key_presence(const T *array,int task_start_index,int task_end_index, T  key_to_search);
// For testing
template <typename T>
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	cout<<array_size <<","; //everyone gets its broadcast array size 
    ...
    int task_end_index = task_start_index + each_process_task_size - 1;
     ...
     // Broadcast the actual vector data to all processes from rank=0 process to others, the array is shared as muxh as their task requires
    MPI_Scatter(First_line_input_storage,task_end_index, MPI_CHAR, 0, MPI_COMM_WORLD);
